...
// FeatureCard.jsx
const FeatureCard = ({ icon: Icon, title, description, onClick, to }) => {
  const Component = to ? NavLink : 'button';
  const props = to ? { to } : { onClick };

  return (
    <Component {...props} className="group no-underline">
      <div className="p-6 rounded-2xl bg-white/5 backdrop-blur-sm border-4 border-white/10 hover:border-white hover:bg-white/10 transition-all duration-300 transform hover:-translate-y-1">
        <Icon className="w-8 h-8 text-purple-300 mb-4 mx-auto" />
...
// PathModal.jsx
const PathModal = ({ isVisible, isClosing, onClose }) => {
  if (!isVisible) return null;
...
const Hero = () => {
  ...
  const features = [
    { icon: Library, title: "Buy Now", description: "Explore our vast collection of books", to: "/products" },
    { icon: BookOpen, title: "E-Books", description: "Digital reading experience", to: "/ebooks" },
    { icon: Send, title: "Sell Now", description: "Turn your books into cash", onClick: () => openModal() },
    { icon: BookOpen, title: "Rent Books", description: "Enables you to rent books for a short period", to: "/rent" },
    { icon: BookCopy, title: "Donate Now", description: "Donate books to those in need", to: "/donate", className: "col-span-1 lg:col-span-2" },
    { icon: BookCopy, title: "Manga", description: "Japanese comic collection", to: "/manga", className: "col-span-1 lg:col-span-2" }
  ];
      ...
      <div className="relative z-10 container mx-auto px-4">
        <div className="min-h-screen flex flex-col items-center justify-center">
          <div className="text-center max-w-4xl mx-auto mb-16">
            <h1 className="text-5xl md:text-7xl font-bold mt-4 text-[#FFD700] mb-8 animate-text-shine">
              Bookefy
            </h1>
